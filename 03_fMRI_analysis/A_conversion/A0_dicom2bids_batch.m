...
addpath(genpath('...\A_dicm2bids'));

sourceF = '...\IMACU_raw\sorted\'; %this is the folder that contains your raw data (folders eg. ccnb_12345)
targetF = '...\IMACU\Data\';  % C:\Users\saraw\Desktop\SEBs\multi_stim

addpath('...\A_dicm2bids\dicm2nii-master')
...
if individ_ses_length == 0
    ...
    if sess(1) == 1 && sess(2) == length(pb) %% 1 sj and a lot of seesions to store in diff folders
        ...
        for ses = start_ses:sess(2)
            fSess = [fSubj 'ses-' num2str(ses) '/'];
            if ~exist(fSess, 'dir'), mkdir(fSess); end
            dcmDir = [sourceF filesep pb(ses).name];
            A1_dicm2bids(dcmDir, targetF, 1, task, sess, ses, skip_sbref);    %%%% übergib session: hier halten wir uns an session-ordner & benennung! irgendwie...
        end
    ...
    elseif sess(2) > 1
        if sess(2) == length(pb)
            ...
            for subj = start_sub:numSJ
                ...
                for ses = start_ses:sess(2)
                    dcmDir = [sourceF filesep pb(ses+(subj-1)*(sess(2))).name];
                    A1_dicm2bids(dcmDir, targetF, subj, task, sess, ses, skip_sbref);    %%%% übergib session: hier halten wir uns an session-ordner & benennung! irgendwie...
                end
            end
        end
    else
        for subj = start_sub:length(pb) %% subj must be numeric
            if subj < 10
                    sub = ['00' num2str(subj)];
                elseif subj < 100
                    sub = ['0' num2str(subj)];
