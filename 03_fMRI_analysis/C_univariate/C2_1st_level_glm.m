function D2_1st_level_glm(SJs, sj, subj_dir, outputfolder_1st, prefix_func, tr, fmri_t, fmri_t0, hpf, runs,                                             condnames, onsets,       duration,  hm, cc, mask, responses)
...
for r = 1:size(runs,2)
    ...
    if exist(run_dir)==7
        ...
        if hm
            ...
            f1=spm_select('List', run_dir, ['^rp_' prefix_func(n:end) runs{sj, r}(1:k) '.txt']);
            while isempty(f1)
                n=n+1;
                f1=spm_select('List', run_dir, ['^rp_' prefix_func(n:end) runs{sj, r}(1:k) '.txt']);
            end
        ...
        for c = 1:length(condnames)
            ...
            % matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).duration = duration;            
             ...
             if size(duration) == 1
                 ...
                 matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).duration = duration{r,c};
             end
